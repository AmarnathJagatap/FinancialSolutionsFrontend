...
import { homebankname } from "./HomeLoan";
import { businessbankname } from "./BusinessLoan";
import { personalbankname } from "./PersonalLoan";
...
export default function EditFile() {
  ...
  const [referencename, setreferencename] = React.useState(
    location.state.fileDetails.referencename
  );
  const [product, setProduct] = React.useState(
    location.state.fileDetails.product
  );
  const [place, setPlace] = React.useState(location.state.fileDetails.place);
  const [mobileno, setMobileno] = React.useState(
    location.state.fileDetails.mobileno
  );
  const [loanamount, setloanamount] = React.useState(
    location.state.fileDetails.loanamount
  );
  const [customername, setcustomername] = React.useState(
    location.state.fileDetails.customername
  );
  const [status, setStatus] = React.useState(location.state.fileDetails.status);
  const [filetype, setFileType] = React.useState(
    location.state.fileDetails.filetype
  );
  const [presentbankname, setpresentbankname] = React.useState(
    location.state.fileDetails.presentbankname
  );
  const [bankemployeename, setbankemployeename] = React.useState(
    location.state.fileDetails.bankemployeename
  );
  const [logindate, setlogindate] = React.useState(
    location.state.fileDetails.logindate
  );
  const [approveddate, setapproveddate] = React.useState(
    location.state.fileDetails.approveddate
  );
  const [rejeceddate, setrejeceddate] = React.useState(
    location.state.fileDetails.rejeceddate
  );
  const [disburseddate, setdisburseddate] = React.useState(
    location.state.fileDetails.disburseddate
  );
  const [techincaldate, settechincaldate] = React.useState(
    location.state.fileDetails.techincaldate
  );
  const [techincalvaluvation, settechincalvaluvation] = React.useState(
    location.state.fileDetails.techincalvaluvation
  );
  ...
  const editfileData = async () => {
    ...
    await fetch(
      Apilink + `/files/getfiledata/${location.state.fileDetails.id}`,
      {
        method: "PUT",
        body: JSON.stringify({
        ...
        >
          ...
          <div>
            ...
            <TextField
              ...
              value={presentbankname} // Use "value" instead of "defaultValue"
            >
              {product === "Business Loan"
                ? businessbankname.map((option) => (
                    <MenuItem key={option.value} value={option.value}>
                      {option.label}
                    </MenuItem>
                  ))
                : product === "Personal Loan"
                ? personalbankname.map((option) => (
                    <MenuItem key={option.value} value={option.value}>
                      {option.label}
                    </MenuItem>
                  ))
                : product === "LAP(Mortgage Loan)"
                ? homebankname.map((option) => (
                    <MenuItem key={option.value} value={option.value}>
                      {option.label}
                    </MenuItem>
                  ))
